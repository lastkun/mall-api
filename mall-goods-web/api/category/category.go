...
func List(ctx *gin.Context) {
	r, err := global.GoodsServiceClient.GetAllCategorysList(context.Background(), &empty.Empty{})
	if err != nil {
...
func Detail(ctx *gin.Context) {
	...
	if r, err := global.GoodsServiceClient.GetSubCategory(context.Background(), &proto.CategoryListRequest{
		Id: int32(i),
	}); err != nil {
		base.HandleGRPCErrorToHttp(err, ctx)
		return
...
func New(ctx *gin.Context) {
	...
	rsp, err := global.GoodsServiceClient.CreateCategory(context.Background(), &proto.CategoryInfoRequest{
...
func Delete(ctx *gin.Context) {
	...
	//2. 将所有的分类全部逻辑删除
	//3. 将该分类下的所有的商品逻辑删除
	_, err = global.GoodsServiceClient.DeleteCategory(context.Background(), &proto.DeleteCategoryRequest{Id: int32(i)})
	if err != nil {
		base.HandleGRPCErrorToHttp(err, ctx)
		return
	}
...
func Update(ctx *gin.Context) {
	...
	_, err = global.GoodsServiceClient.UpdateCategory(context.Background(), request)
	if err != nil {
		base.HandleGRPCErrorToHttp(err, ctx)
		return
	}
